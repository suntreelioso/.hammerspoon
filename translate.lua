...
3. Features a "Copy Translation" button in the bottom-right.
4. Fixed 400*300 size, supports Esc key to close.
]]--
...
local function fetchTranslation(text, callback)
    ...
    print("[Translate] Fetching URL: " .. url)
    hs.http.asyncGet(url, nil, function(status, body)
        print("[Translate] HTTP status: " .. tostring(status))
        if status ~= 200 then
            local errMsg = "Translation Error (HTTP " .. tostring(status) .. ")"
            if status == 0 or status == -1 then
                errMsg = "Network Error: Cannot reach Google Translate.\nPlease check your internet connection."
            end
            callback(errMsg)
            return
        end
        ...
        if not ok or not response or not response[1] then
            print("[Translate] JSON parse failed. body: " .. tostring(body))
            callback("Failed to parse response data")
            return
        end
        local result = ""
...
local function translateSelection()
    ...
    print("[Translate] Selected text via UI: " .. tostring(text))

    if not text or text == "" then
        -- Simulate Cmd+C and wait for clipboard update
        hs.eventtap.keyStroke({"cmd"}, "c")
        hs.timer.doAfter(0.5, function()
            local newText = hs.pasteboard.getContents()
            print("[Translate] Old clipboard: " .. tostring(oldClipboard))
            print("[Translate] New clipboard: " .. tostring(newText))

            -- Use new clipboard content if it changed, otherwise try old content
            local textToTranslate = nil
            if newText and newText ~= "" and newText ~= oldClipboard then
                textToTranslate = newText
            elseif newText and newText ~= "" then
                -- Clipboard didn't change; might be same selection, translate anyway
                textToTranslate = newText
            end

            if textToTranslate then
                showPopup("Translating...")
                fetchTranslation(textToTranslate, function(res)
                    if transWebView then 
                        lastTranslation = res
                ...
                print("[Translate] No text found in clipboard, aborting.")
            end
        end)
    else
        showPopup("Translating...")
